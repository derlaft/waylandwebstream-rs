...
use wayland_client::{
    protocol::{wl_buffer, wl_compositor, wl_pointer, wl_registry, wl_seat, wl_shm, wl_shm_pool, wl_surface},
...
fn main() {
    println!("Starting Wayland pointer-reactive test client...");
    ...
    // click is just an attach -- no per-frame pixel fill.
    let stride = WIDTH * 4;
    ...
    println!("Pointer test client ready: black=idle, white=button held");

    let run_secs: u64 = std::env::var("POINTER_CLIENT_RUN_SECS")
        .ok()
    ...
    println!("Pointer test client exiting");
}
...
struct AppState {
    ...
    is_white: bool,
    /// Set by `wl_pointer::Button`, applied to the surface once the batch of
    /// events is closed by `wl_pointer::Frame`.
    ...
    fn event(state: &mut Self, _: &wl_pointer::WlPointer, event: wl_pointer::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match event {
            ...
            wl_pointer::Event::Button { button, state: button_state, .. } => {
                ...
                println!("pointer button {} {}", button, if pressed { "down" } else { "up" });
                state.pending_white = Some(pressed);
            }
