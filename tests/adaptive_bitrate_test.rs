...
fn congestion_cuts_bitrate_multiplicatively() {
    ...
    let new_rate = algo.on_send_backlog(t0).expect("should cut");
...
fn congestion_after_cooldown_cuts_again() {
    ...
    let first_cut = algo.on_send_backlog(t0).expect("should cut");
    let second_cut = algo
        .on_send_backlog(t0 + Duration::from_secs(3))
...
fn growth_holds_during_post_cut_cooldown() {
    ...
    algo.on_send_backlog(t0).expect("should cut");
...
fn switches_to_additive_increase_after_a_cut() {
    ...
    algo.on_send_backlog(t0 + Duration::from_secs(2));
